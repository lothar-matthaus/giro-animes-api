...
namespace Giro.Animes.Infra
{
    public class ApplicationUser : IApplicationUser
    {
        ...
        public ApplicationUser(IHttpContextAccessor contextAccessor)
        {
            ...
            InterfaceLanguage = _context.HttpContext.Request.Headers["Interface-Language"].ToString();
            AudioAnimeLanguages = _context.HttpContext.Request.Headers["Audio-Anime-Languages"].ToString().Split(',').Select(x => x.Trim()).ToArray();
            SubtitleAnimeLanguages = _context.HttpContext.Request.Headers["Subtitle-Anime-Languages"].ToString().Split(',').Select(x => x.Trim()).ToArray();
        }
