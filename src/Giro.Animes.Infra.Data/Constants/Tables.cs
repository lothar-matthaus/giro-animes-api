namespace Giro.Animes.Infra.Data.Constants
{
    public static class Tables
    {
        public static class Common
        {
            public const string AUTHORS = "authors";
            public const string GENRES = "genres";
            public const string LANGUAGES = "languages";
            public const string STUDIOS = "studios";
            public const string BIOGRAPHIES = "biographies";
        }

        public static class Content
        {
            public const string ANIMES = "animes";
            public const string ANIME_SCREENSHOTS = "anime_screenshots";
            public const string ANIME_TITLES = "anime_titles";
            public const string AVATARS = "avatars";
            public const string COVERS = "covers";
            public const string EPISODES = "episodes";
            public const string EPISODE_FILES = "episode_files";
            public const string EPISODE_TITLES = "episode_titles";
            public const string EPISODE_SINOPSES = "episode_sinopses";
            public const string GENRE_DESCRIPTIONS = "genre_descriptions";
            public const string GENRE_TITLES = "genre_titles";
            public const string ANIMES_GENRES = "animes_genres";
            public const string LOGOS = "logos";
            public const string ANIME_SINOPSES = "anime_sinopses";
            public const string AUTHOR_WORKS = "author_works";
            public const string WATCHLIST = "watchlist";
            public const string RATINGS = "ratings";
            public const string SETTINGS_ANIME_AUDIO_LANGUAGES = "settings_anime_audio_languages";
            public const string SETTINGS_ANIME_SUBTITLE_LANGUAGES = "settings_anime_subtitle_languages";
            public const string EPISODE_LANGUAGES = "episode_languages";
        }

        public static class Management
        {
            public const string ACCOUNTS = "accounts";
            public const string USERS = "users";
            public const string SETTINGS = "settings";
        }

        public static class Misc
        {
            public const string AUDIT_LOGS = "audit_logs";
        }
    }
}
